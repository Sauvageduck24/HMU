...
class ROCStoriesLoader:
    def __init__(self, split='train', max_length=32, tokenizer_name='t5-small'):
        self.dataset = load_dataset('mintujupally/ROCStories', split=split)
    ...
    def get_data(self) -> List[Dict[str, torch.Tensor]]:
        ...
        for example in self.dataset:
            # El dataset puede tener los campos 'sentence1', ..., 'sentence5'
            sentences = [example.get(f'sentence{i}', '') for i in range(1, 6)]
